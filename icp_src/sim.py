...
class PyBulletSim:
    ...
    def move_tool(self, position, orientation, speed=0.03):
        ...
        res = p.calculateInverseKinematics(self.robot_body_id, self.robot_end_effector_link_index, position, orientation, maxNumIterations = 50)
        for i in range(len(target_joint_state)):
            target_joint_state[i] = res[i]
