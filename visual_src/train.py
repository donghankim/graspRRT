...
def get_center_angle(
    ...
    x = round((left_coord[0] + right_coord[0])/2)
    y = round((left_coord[1] + right_coord[1])/2)
    center_coord = np.array([x,y], dtype = np.float32)
    angle = np.arctan2(right_coord[1] - left_coord[1], right_coord[0] - left_coord[0])
...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, idx: int
        ...
        click_pt = data_torch['center_point'].numpy()
        lc, rc = get_finger_points(click_pt, data_torch['angle'].item())
        kps = KeypointsOnImage([
            Keypoint(x = lc[0], y = lc[1]),
            Keypoint(x = rc[0], y = rc[1])], shape = rgb.shape[:2])
        img_aug, kps_aug = self.aug_pipeline(image = rgb, keypoints = kps)
        lc_aug = kps_aug[0].xy
        rc_aug = kps_aug[1].xy
        center, angle = get_center_angle(lc_aug, rc_aug)
        deg = np.degrees(angle)

        data_torch['rgb'] = torch.from_numpy(img_aug) # look into this next
        data_torch['center_point'] = torch.from_numpy(center).type(torch.float32)
        data_torch['angle'] = torch.tensor(deg).type(torch.float32)
        return data_torch
